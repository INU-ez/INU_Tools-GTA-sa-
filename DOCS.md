...
- Нажмите **Извлечь ресурсы** — извлечёт DFF, COL и текстуры из IMG архивов в `.inu_cache/`
- Долго, но только один раз
...
**Авто-LOD:** при добавлении пары DFF+LOD, LOD получает ID+1, дальность+50, тот же TXD.
...
| Удалить | `gtatools.remove_ipl` | Удалить по Model ID (пересчёт LOD индексов) |
...
- **Путь к системным текстурам** — общая папка текстур
- **Папка .blend** — авто-установка на текущий .blend
- **Загрузить текстуры** — поиск и привязка по именам материалов
- **Drag & Drop** — перетаскивание изображений во вьюпорт для создания материалов
...
| Кнопка | Описание |
|--------|----------|
| Импорт NODES.dat | Загрузка бинарных нод |
| Экспорт NODES.dat | Сохранение скомпилированных нод |
...
| Кнопка | Описание |
|--------|----------|
| Apply Quickstart | Применить Itera Quickstart материал |
| Fix Collection | Автоорганизация Itera материалов |
| Remove Itera | Удалить Itera материалы и восстановить оригиналы |

Автодетект библиотеки Itera Tools 3 в Asset Libraries. Панель видна только когда Itera установлена.

### Lightmap Generator (MTA)

**Панель:** View3D → Sidebar (N) → GTA Tools → Lightmap Generator (beta)

Генерация Lua кода для MTA SA скриптов лайтмапов.

| Кнопка | Оператор | Описание |
|--------|----------|----------|
| Загрузить лайтмап | `gtatools.load_lightmap` | Загрузить изображение лайтмапа |
...
**Поля:** путь к лайтмапу, Model ID. Результат: Lua таблица с именами текстур и ссылками на лайтмап.

### Pipeline

Выбор рендер-пайплайна на объект:
- **None** — без пайплайна
- **Building** (0x53F2009A) — Day/Night vertex colors
- **Reflections** (0x53F20098) — отражения на окнах
