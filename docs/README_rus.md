...
### 🚧 Войдёт в 2.3.0 (готово — ждёт релиза)

- 🔗 **Мост с Ariane** — живой мост слежения за папкой: авто-импортит DFF, которые внешний инструмент **Ariane** кидает в открытый Blender
...
- 📷 **Камеры** — импорт/экспорт camera `.dat` (позиции + FOV/roll как keyframes)
- 🧩 **Фрагменты** — разбивка меша на breakable-фрагменты одной кнопкой (сетка / разброс)
- ✋ **Жесты рук** — авторинг бандитских жестов для `ghands.ifp`
- 💡 **Прилайт — все типы ламп + HDRI** — запекание теперь берёт **Point / Sun / Spot / Area** (тумблеры по источникам) и умеет сэмплить окружение **World / HDRI** по нормали, а не только точечные лампы
- 🔥 **Запекание текстур** — **карта Alpha** (RGBA), **Декаль** (Shadow → прозрачная тень-декаль: порог/мягкость, жёсткая граница, инверсия), **RGBA при сохранении одной карты**, **«Свет от сцены»** для Shadow/Diffuse-Lit, **Bevel по выделенным рёбрам**, тумблер **прозрачный / средний цвет фона**
- 📐 **UV-инструменты** — **Масштаб островов в сетку** + тексель-плотность
...
- 🗺️ **Импорт: ваниль vs кастом** — галочка **«Стандартная модель GTA SA (vanilla)»**; ВЫКЛ = кастом (связать геометрию, сохранить двусторонние заборы), ВКЛ не трогает ванильное поведение
...
- 💡 **2DFX** — 4 новых типа с превью: **Дорожный знак**, **Вход/Выход**, **Эскалатор**, **Raw**
- 🎬 **Анимации (IFP)** — **зеркало L/R** с учётом rest-позы; байт-точный round-trip + фиксы дёрганья; **IFP Library Viewer** (живой скраб 294 ванильных анимаций через дропдаун Action + таймлайн); подпанель **«Иерархия фреймов»** со сменой родителя в дереве
- 🧱 **Разбор импорта IDE / IPL / IMG** — модальный импорт с прогресс-баром, скан **«Найти IMG»** по папке игры, мульти-выбор IPL, авто-заполнение **имя TXD / LOD-партнёр / дистанции / связь с IDE**; **копия модели добавляется НОВЫМ инстансом IPL**, а не перезаписывает исходную строку; разделение **«Обновить из IDE» / «из IPL»**; **Импорт / Экспорт** переключающимися вкладками
- 🌊 **Вода** — оверлей «Лимиты воды» + привязка к блоку; **«Порезать по блокам (500)»** ещё и раскидывает куски по объектам
- 🧵 **TXD** — трилинейный **мип-фильтр** на экспортируемых текстурах
- 🧱 **Коллизии** — импорт в отдельную коллекцию, дамми-фреймы кубиками
- ⚡ **Производительность** — мемоизация отрисовки панелей (`draw_cache`), убирает лаги вьюпорта на больших картах
- 🐛 **Фиксы** — больше не показывает русский в английском Blender; плавающие окна больше не воруют клики, предназначенные N-панели / тулбару поверх них
