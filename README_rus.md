...
Что в планах на следующие релизы, по приоритетам:

**Высокий приоритет**
- 🗺️ **Map Export** — единый экспорт сцены в IPL + IDE + COL + TXD одной кнопкой с авто-парингом LOD и пулом ID

**Средний приоритет**
- 💾 **Binary IPL Write** — сохранение IPL в бинарном формате `bnry` (чтение уже есть)
- 🎞️ **UV-анимация в DFF** — запись анимированных UV материалов прямо в бинарь DFF (чанки `0x2B` / `0x135`)
- 🎬 **IFP Batch Import** — range-применение папки с анимациями к armature через NLA-треки
- 🖼️ **Bitmaps Manager** — отчёт о недостающих текстурах, batch-копирование в папку, поиск дубликатов

**Низкий приоритет**
- 🚗 **Машины (Phase 2+)** — color slots (Primary / Secondary / Headlight), damage-дамми, vehicle env-map пресеты
- 🎨 **Custom GTA Material** — панель материала с vehicle colors, blend modes, пресетами в один клик
- 💥 **Breakable Objects** — разрушаемые меши (чанк `0x253F2FD` в DFF)
- 🛡️ **DFF Lock** — блокировка DFF в стиле ZModeler для защищённых релизов
- 🪨 **CST IO** — импорт/экспорт формата Steve's COL Editor
- 🚂 **Train Path Splines** — редактирование `tracks.dat` как Blender-кривых со станциями
- 📏 **Vehicle Scale Helper** — пропорциональное масштабирование иерархии машины
- 🚧 **Roadblocks & Traffic Lights** — расширенные флаги path-нодов
- 🛣️ **FLA4 Path Format** — расширенный формат нодов для Fastman Limit Adjuster 4
