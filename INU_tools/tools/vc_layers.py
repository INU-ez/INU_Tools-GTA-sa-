    ...
    def draw_item(self, context, layout, data, item, icon, active_data,
        ...
        if self.layout_type in {'DEFAULT', 'COMPACT'}:
            ...
            row.prop(item, "selected", text="", emboss=False,
                     **inu_icon(safe_icon('CHECKBOX_HLT') if item.selected else 'CHECKBOX_DEHLT'))
            row.prop(item, "visible", text="", emboss=False,
                     **inu_icon(safe_icon('HIDE_OFF') if item.visible else 'HIDE_ON'))
            row.prop(item, "locked", text="", emboss=False,
                     **inu_icon(safe_icon('LOCKED') if item.locked else 'UNLOCKED'))
...
def _draw_other_attrs_flat_list(layout, mesh):
    ...
    box = layout.box()
    box.label(text=T("Дополнительные атрибуты:"), **inu_icon(safe_icon('COLOR')))
    for attr in others:
        ...
        op = row.operator("gtatools.select_color_attribute",
                          text=attr.name, **inu_icon(icon), depress=is_active)
...
def _draw_stack(layout, mesh, scope, header_text, icon):
    ...
    head.label(text=header_text, **inu_icon(icon))
    ...
    op_add = ctrl.operator("gtatools.vcl_add", text="", **inu_icon(safe_icon('ADD')))
    op_add.scope = scope
    op_add.label = ""
    ctrl.operator("gtatools.vcl_remove", text="", **inu_icon(safe_icon('REMOVE')))
    ctrl.separator()
    op_up = ctrl.operator("gtatools.vcl_move", text="", **inu_icon(safe_icon('TRIA_UP')))
    op_up.direction = 'UP'
    op_dn = ctrl.operator("gtatools.vcl_move", text="", **inu_icon(safe_icon('TRIA_DOWN')))
    op_dn.direction = 'DOWN'

    if count == 0:
        box.label(text=T("Стек пуст — жми +"), **inu_icon(safe_icon('INFO')))
        return
...
def draw_vc_layers_section(layout, context, mesh):
    ...
    if not compat.HAS_COLOR_ATTRIBUTES:
        box = layout.box()
        col = box.column(align=True)
        col.label(text=T("VC Layers System"), **inu_icon(safe_icon('COLOR')))
        col.label(
    ...
    if selected:
        ...
        box.label(text=T("Выделенные слои ({}):").format(len(selected)),
                  **inu_icon(safe_icon('SELECT_EXTEND')))
        row = box.row(align=True)
