...
def _trace_chains(adj, nodes):
    ...
    Returns list of [(x,y,z), ...] coord paths for each chain.
    See `_build_nodes_viz_curves` docstring for algorithm details."""
    n = len(adj)
    visited = set()
    chains = []
...
def _create_viz_curve_from_chains(name, chains, collection, mat_name, color):
    """Build a curve where each chain is one POLY spline."""
    if not chains:
        return None
    curve = bpy.data.curves.new(name, type='CURVE')
    curve.dimensions = '3D'
