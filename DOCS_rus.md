...
**Шаг 3: Импорт карты**
- Нажмите **Импорт карты** — читает DFF и PNG-текстуры **только из кеша** `.inu_cache/` (никакого обращения к IMG во время импорта)
...
- Типичное время: ~30 секунд на район размера LA
...
> 💡 **Пример — пакетный залив в gta3.img:** у тебя 10 домов готовых к экспорту. Укажи путь `gta3.img` в Import Map settings → выдели дома → **Экспорт в IMG** → откроется диалог с UIList где видно TXD-имена всех моделей (редактируемые). Нажал OK — все DFF+COL+TXD записались в архив. После этого обязательно **Rebuild Archive** в IMG-туле (иначе в игре останутся старые версии).
